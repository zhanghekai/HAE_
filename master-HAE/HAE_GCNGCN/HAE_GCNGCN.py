...
check_file = 'zhanghekai/DBLP/compairGCNGCN/premodel/'
...
alllabel = np.load('zhanghekai/DBLP/compairGCNGCN/one_hot_labels.npy')
train_data = np.load('zhanghekai/DBLP/compairGCNGCN/train_idx.npy')
test_data = np.load('zhanghekai/DBLP/compairGCNGCN/test_idx.npy')
...
class GCNPair(object):
    def __init__(self, session,
        ...
        self.l2 = tf.contrib.layers.apply_regularization(tf.contrib.layers.l2_regularizer(0.0001), tf.trainable_variables())
        ...
        #capped_gradients = [(tf.clip_by_value(grad*100, -5., 5.), var) for grad, var in self.gradients if grad is not None]
...
if __name__ == "__main__":
    ...
    xdata = np.load("zhanghekai/DBLP/compairGCNGCN/features.npy")
    #xdata = preprocess_features_v1(xdata)

    adj_data = np.load("zhanghekai/DBLP/compairGCNGCN/small_adj_data.npy")
    with tf.Session() as sess:
        ...
        for epoch in range(epoch_num):
            ...
            with open('zhanghekai/DBLP/compairGCNGCN/premodel/output.txt', 'a+') as fi:
            ...
            if eva_acc > max_acc:
                ...
                np.save('zhanghekai/DBLP/compairGCNGCN/premodel/featts.npy', featts)
